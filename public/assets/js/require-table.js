        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                table.on('refresh.bs.table', function (e, settings, data) {
                    $(Table.config.refreshbtn, toolbar).find(".fa").addClass("rt-spin");
                });
                //当双击单元格时
                ...
                table.on('post-body.bs.table', function (e, settings, json, xhr) {
                    $(Table.config.refreshbtn, toolbar).find(".fa").removeClass("rt-spin");
            ...
            formatter: {
                icon: function (value, row, index) {
                    if (!value)
                        return '';
                    value = value.indexOf(" ") > -1 ? value : "fa rt-" + value;
                    //渲染fontawesome图标
                ...
                status: function (value, row, index) {
                    ...
                    //渲染状态
                    var html = '<span class="text-' + color + '"><i class="fa rt-circle"></i> ' + __(value) + '</span>';
                    return html;
                },
                url: function (value, row, index) {
                    return '<div class="input-group input-group-sm" style="width:250px;"><input type="text" class="form-control input-sm" value="' + value + '"><span class="input-group-btn input-group-sm"><a href="' + value + '" target="_blank" class="btn btn-default btn-sm"><i class="fa rt-link"></i></a></span></div>';
                },
                ...
                operate: function (value, row, index) {
                    ...
                    buttons.push({name: 'dragsort', icon: 'fa rt-arrows', classname: 'btn btn-xs btn-primary btn-dragsort'});
                    buttons.push({name: 'edit', icon: 'fa rt-pencil', classname: 'btn btn-xs btn-success btn-editone', url: options.extend.edit_url});
                    buttons.push({name: 'del', icon: 'fa rt-trash', classname: 'btn btn-xs btn-danger btn-delone'});
                    var html = [];
