    ...
    var Controller = {
        index: function () {
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        {field: 'id', title: '<a href="javascript:;" class="btn btn-success btn-xs btn-toggle"><i class="fa rt-chevron-up"></i></a>', operate: false, formatter: Controller.api.formatter.subnode},
            ...
            $(document.body).on("click", ".btn-toggle", function (e) {
                ...
                var that = this;
                var show = $("i", that).hasClass("rt-chevron-down");
                $("i", that).toggleClass("rt-chevron-down", !show);
                $("i", that).toggleClass("rt-chevron-up", show);
            ...
            $(document.body).on("click", ".btn-toggle-all", function (e) {
                var that = this;
                var show = $("i", that).hasClass("rt-plus");
                $("i", that).toggleClass("rt-plus", !show);
                $("i", that).toggleClass("rt-minus", show);
            ...
            formatter: {
                ...
                subnode: function (value, row, index) {
                    return '<a href="javascript:;" data-id="' + row['id'] + '" data-pid="' + row['pid'] + '" class="btn btn-xs '
                            + (row['haschild'] == 1 ? 'btn-success' : 'btn-default disabled') + ' btn-node-sub"><i class="fa rt-sitemap"></i></a>';
                }
            },
            bindevent: function () {
                var iconlist = [];
                ...
                $(document).on('click', '#chooseicon ul li', function () {
                    $("input[name='row[icon]']").val('fa rt-' + $(this).data("font"));
                    Layer.closeAll();
                });
