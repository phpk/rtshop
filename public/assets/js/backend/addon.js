define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'template'], function ($, undefined, Backend, Table, Form, Template) {
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: Config.qya.api_url + '/addon/index',
                    add_url: '',
                    edit_url: '',
                    del_url: '',
                    multi_url: ''
                ...
                if (!userinfo) {
                    Layer.open({
                        ...
                        yes: function (index, layero) {
                            Fast.api.ajax({
                                url: Config.qya.api_url + '/user/login',
                                dataType: 'jsonp',
                        ...
                        success: function (layero, index) {
                            $(".layui-layer-btn1", layero).prop("href", "http://www.qya.cn/user/register.html").prop("target", "_blank");
                        }
                    });
                } else {
                    var userinfo = Controller.api.userinfo.get();
                    ...
                    Layer.open({
                        ...
                        yes: function () {
                            Fast.api.ajax({
                                url: Config.qya.api_url + '/user/logout',
                                dataType: 'jsonp',
        ...
        api: {
            ...
            userinfo: {
                get: function () {
                    var userinfo = localStorage.getItem("qya_userinfo");
                    return userinfo ? JSON.parse(userinfo) : null;
                },
                set: function (data) {
                    if (data) {
                        localStorage.setItem("qya_userinfo", JSON.stringify(data));
                    } else {
                        localStorage.removeItem("qya_userinfo");
                    }
                }
            }
        }
